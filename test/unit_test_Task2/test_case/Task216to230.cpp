...
bool UNIT_TEST_Task2::Task218()
{
    ...
    //! expect ----------------------------------
    // Chi tiết tính toán LF:
    // truckOfArvn: safeCeil((10 * 5 + 0 * 304) / 30) = safeCeil(50 / 30) = 2
    // armoredCarOfArvn: safeCeil((5 * 2 + 3 * 304) / 30) = safeCeil(922 / 30) = 31
    // Tổng LF = 2 + 31 = 33
    //
    // Chi tiết tính toán EXP:
    // specialForceOfArvn:
    //   - Base score = (4 * 56) + (3 * 1) = 224 + 3 = 227
    //   - Logic đặc biệt:
    //     - Vì weight = 1 là số chính phương, cộng thêm 1: 227 + 1 = 228
    //   - EXP = min(228, 500) = 228
    //
    // Kết quả:
    // LF = 33
...
bool UNIT_TEST_Task2::Task221()
{
    ...
    // LF = 61
    // EXP = 2
    string expect = "ARVN[name=TestArmy221,LF=61,EXP=2,unitList=(UnitList[count_vehicle=3;count_infantry=2;"
                    "Infantry[infantryType=SNIPER,quantity=2,weight=1,pos=(1,2)],"
...
bool UNIT_TEST_Task2::Task222()
{
    ...
    // LF = 62
    // EXP = 286
    string expect = "ARVN[name=TestArmy222,LF=62,EXP=286,unitList=(UnitList[count_vehicle=5;count_infantry=3;"
                    "Infantry[infantryType=REGULARINFANTRY,quantity=3,weight=2,pos=(2,3)],"
                    "Vehicle[vehicleType=TANK,quantity=5,weight=3,pos=(1,1)]])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
...
bool UNIT_TEST_Task2::Task223()
{
    ...
    // LF = 51
    // EXP = 65
    string expect = "ARVN[name=TestArmy223,LF=51,EXP=65,unitList=(UnitList[count_vehicle=4;count_infantry=3;"
                    "Infantry[infantryType=ANTIAIRCRAFTSQUAD,quantity=3,weight=3,pos=(3,3)],"
                    "Vehicle[vehicleType=ARTILLERY,quantity=4,weight=2,pos=(1,1)]])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
...
bool UNIT_TEST_Task2::Task224()
{
    ...
    // LF = 62
    // EXP = 254
    string expect = "ARVN[name=TestArmy224,LF=62,EXP=254,unitList=(UnitList[count_vehicle=2;count_infantry=6;"
                    "Infantry[infantryType=SPECIALFORCES,quantity=6,weight=5,pos=(3,3)],"
                    "Vehicle[vehicleType=TANK,quantity=2,weight=4,pos=(2,2)]])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
...
bool UNIT_TEST_Task2::Task225()
{
    ...
    // LF = 94
    // EXP = 254
    string expect = "ARVN[name=TestArmy225,LF=94,EXP=254,unitList=(UnitList[count_vehicle=9;count_infantry=6;"
                    "Infantry[infantryType=SPECIALFORCES,quantity=6,weight=5,pos=(3,3)],"
                    "Vehicle[vehicleType=TANK,quantity=2,weight=4,pos=(2,2)],"
                    "Vehicle[vehicleType=TRUCK,quantity=3,weight=2,pos=(3,1)],"
                    "Vehicle[vehicleType=ARMOREDCAR,quantity=4,weight=3,pos=(1,1)]])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
...
bool UNIT_TEST_Task2::Task228()
{
    ...
    string expect = "ARVN[name=EmptyARVN,LF=0,EXP=0,unitList=(UnitList[count_vehicle=0;count_infantry=0;])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
...
bool UNIT_TEST_Task2::Task229()
{
    ...
    // LF = 63
    // EXP = 12
    string expect = "ARVN[name=ARVN,LF=63,EXP=12,unitList=(UnitList[count_vehicle=6;count_infantry=6;"
                    "Infantry[infantryType=SNIPER,quantity=6,weight=2,pos=(5,5)],"
                    "Vehicle[vehicleType=TANK,quantity=4,weight=2,pos=(3,3)],"
                    "Vehicle[vehicleType=TRUCK,quantity=2,weight=1,pos=(4,4)]])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
...
bool UNIT_TEST_Task2::Task230()
{
    ...
    // LF = 63
    // EXP = 8
    string expect = "ARVN[name=ARVN,LF=63,EXP=8,unitList=(UnitList[count_vehicle=5;count_infantry=4;"
                    "Infantry[infantryType=SNIPER,quantity=4,weight=2,pos=(5,5)],"
                    "Vehicle[vehicleType=TANK,quantity=3,weight=3,pos=(3,3)],"
                    "Vehicle[vehicleType=TRUCK,quantity=2,weight=2,pos=(4,4)]])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
...
bool UNIT_TEST_Task2::Task226()
{
    ...
    // LF = 63
    // EXP = 10
    string expect = "ARVN[name=ARVN,LF=63,EXP=10,unitList=(UnitList[count_vehicle=8;count_infantry=5;"
                    "Infantry[infantryType=SNIPER,quantity=5,weight=2,pos=(3,3)],"
                    "Vehicle[vehicleType=TANK,quantity=5,weight=2,pos=(3,2)],"
                    "Vehicle[vehicleType=TRUCK,quantity=3,weight=1,pos=(3,1)]])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
...
bool UNIT_TEST_Task2::Task227()
{
    ...
    // LF = 63
    // EXP = 10
    string expect = "ARVN[name=ARVN,LF=63,EXP=10,unitList=(UnitList[count_vehicle=7;count_infantry=5;"
                    "Infantry[infantryType=SNIPER,quantity=5,weight=2,pos=(3,3)],"
                    "Vehicle[vehicleType=TANK,quantity=4,weight=2,pos=(3,2)],"
                    "Vehicle[vehicleType=TRUCK,quantity=3,weight=1,pos=(3,1)]])]";
    //! output ----------------------------------
    stringstream output;
    output << result;
